(() => {
  ...
  async function loadLocale() {
    try { const lang=window.MercatoUI?.getLanguage?.()||'en',r=await fetch(`/locales/${lang==='fa'?'fa':'en'}.json`,{cache:'no-cache'});state.locale=r.ok?await r.json():{} } catch { state.locale={} }
  }
  const tr = text => state.locale[String(text)] || window.MercatoUI?.t?.(text) || String(text);

  async function loadPublicSettings() {
    try { const r=await fetch('/api/settings/public',{cache:'no-store'});if(r.ok)state.publicSettings={...state.publicSettings,...(await r.json())} } catch {}
  ...
  function setupLoginArtwork() {
    ...
    if(isPos){view.classList.add('mercato-pos-login');const hero=document.createElement('div');hero.className='login-hero';const shell=document.createElement('div');shell.className='login-form-shell';while(view.firstChild)shell.appendChild(view.firstChild);view.append(hero,shell)}
  }

  function setupLoginIdentity() {
    const input=isAdmin?$('loginEmail'):$('email');if(!input)return;input.type=state.publicSettings.useUsername?'text':'email';input.autocomplete='username';
    const label=isAdmin?input.closest('label'):input.previousElementSibling,text=state.publicSettings.useUsername?tr('Username'):tr('Email');
    if(isAdmin&&label?.firstChild?.nodeType===Node.TEXT_NODE)label.firstChild.nodeValue=text;else if(isPos&&label)label.textContent=text;input.placeholder=text;
  }

  function currentStoredUser() {
    try {
      if(isAdmin)return JSON.parse(localStorage.getItem('mercato.admin.session')||'null')?.user||null;
      if(isPos)return JSON.parse(sessionStorage.getItem('mercato.pos.user')||'null');
    } catch {}
    return null;
  }
  function syncIdentityDisplay() {
    const user=currentStoredUser();if(!user)return;const identity=state.publicSettings.useUsername?(user.username||user.email):(user.email||user.username);
    if(isAdmin&&$('whoEmail'))$('whoEmail').textContent=identity||'';
    if(isPos&&$('staffLabel'))$('staffLabel').textContent=`${identity||'Staff'} · ${user.role||''}`;
  }
  ...
  function setupAdminRouting() {
    ...
    const wrapped=function(page){original(page);if(!fromHash)history.replaceState(null,'',`/admin/#/${encodeURIComponent(page)}`);document.title=`${document.querySelector(`#page-${CSS.escape(page)} h1`)?.textContent||'Back Office'} · Mercato`;syncIdentityDisplay()};
    wrapped.__routed=true;window.showPage=wrapped;
    const route=()=>{const page=decodeURIComponent(location.hash.match(/^#\/([^/?#]+)/)?.[1]||'dashboard');fromHash=true;try{wrapped(page)}finally{fromHash=false}};addEventListener('hashchange',route);
    if(typeof window.signIn==='function'){const originalSignIn=window.signIn;window.signIn=function(){try{const session=JSON.parse(localStorage.getItem('mercato.admin.session')||'null');if(!session?.user?.canAccessBackOffice){window.signOut?.();if($('loginError'))$('loginError').textContent=tr('Back Office access is not enabled for this staff member.');return}}catch{}originalSignIn();syncIdentityDisplay();route()}}
    if(!$('appView')?.classList.contains('hidden'))route();
  }
  ...
  function setupProductImageUpload(){
    if(!isAdmin)return;const url=$('productImage'),form=$('productForm');if(!url||!form||form.dataset.mediaFixed)return;form.dataset.mediaFixed='1';const label=url.closest('label');url.type='hidden';if(label?.firstChild?.nodeType===Node.TEXT_NODE)label.firstChild.nodeValue=tr('Product image');
    const file=document.createElement('input');file.id='productImageFile';file.className='field';file.type='file';file.accept='image/jpeg,image/png,image/webp';label?.appendChild(file);const preview=document.createElement('div');preview.className='media-preview';label?.appendChild(preview);const show=src=>preview.innerHTML=src?`<img src="${esc(src)}" alt=""><span class="field-help">${esc(tr('Current image'))}</span>`:'';file.onchange=()=>show(file.files?.[0]?URL.createObjectURL(file.files[0]):url.value);
    if(typeof window.editProduct==='function'){const old=window.editProduct;window.editProduct=id=>{old(id);file.value='';show(url.value)}}if(typeof window.resetProductForm==='function'){const old=window.resetProductForm;window.resetProductForm=()=>{old();file.value='';show('')}}
    form.onsubmit=async e=>{e.preventDefault();try{let imageUrl=url.value||null;if(file.files?.[0]){const session=JSON.parse(localStorage.getItem('mercato.admin.session')||'null'),fd=new FormData();fd.append('file',file.files[0]);const r=await fetch('/api/media/product-image',{method:'POST',headers:{Authorization:`Bearer ${session?.token||''}`},body:fd}),data=await r.json();if(!r.ok)throw new Error(data?.error||`Upload failed (${r.status})`);imageUrl=data.thumbnailUrl||data.imageUrl}const id=$('productId').value;await window.api('/api/products'+(id?'/'+id:''),{method:id?'PUT':'POST',body:JSON.stringify({name:$('productName').value,sku:$('productSku').value||null,imageUrl,purchasePrice:Number($('productPurchase').value),salePrice:Number($('productSale').value),categoryId:$('productCategory').value||null,artistId:$('productArtist').value||null})});window.resetProductForm?.();await window.loadProducts?.();window.toast?.(tr('Saved'))}catch(error){window.toast?.(error.message,true)}};
  }
  ...
  function ensurePrintFooter(root=document){
    const docs=[];if(root.matches?.('.doc,.printable .receipt'))docs.push(root);root.querySelectorAll?.('.printable .doc,.printable .receipt,.doc').forEach(x=>docs.push(x));
    docs.forEach(doc=>{if(doc.querySelector(':scope > .print-powered'))return;const footer=document.createElement('div');footer.className='print-powered';footer.innerHTML='Powered by <a href="https://badje.ir">badje.ir</a>';doc.appendChild(footer)});
  }

  function setupSettings(){
    if(!isAdmin)return;const form=$('generalSettingsForm');if(!form||form.dataset.identityFixed)return;form.dataset.identityFixed='1';const target=$('showImages')?.closest('label');const label=document.createElement('label');label.style.alignContent='end';label.innerHTML=`<span><input id="useUsername" type="checkbox"> ${esc(tr('Use username for staff login'))}</span><span class="field-help">${esc(tr('When enabled, username is the login identity. When disabled, email is required for login.'))}</span>`;target?.after(label);
    if(typeof window.loadSettings==='function'){const old=window.loadSettings;window.loadSettings=async function(){const result=await old();try{const latest=await window.api('/api/settings');state.publicSettings.useUsername=!!latest.useUsername;if($('useUsername'))$('useUsername').checked=!!latest.useUsername}catch{}return result}}
    form.onsubmit=async e=>{e.preventDefault();try{await window.api('/api/settings',{method:'PUT',body:JSON.stringify({systemLanguage:$('systemLanguage').value,posShowProductImages:$('showImages').checked,useUsername:$('useUsername').checked})});state.publicSettings.useUsername=$('useUsername').checked;state.publicSettings.systemLanguage=$('systemLanguage').value;await window.MercatoUI?.setLanguage?.($('systemLanguage').value);await loadLocale();setupLoginIdentity();syncIdentityDisplay();window.toast?.(tr('Saved'))}catch(error){window.toast?.(error.message,true)}};
  }

  function setupStaff(){
    if(!isAdmin)return;const form=$('staffForm');if(!form||form.dataset.identityFixed)return;form.dataset.identityFixed='1';const email=$('staffEmail'),emailLabel=email?.closest('label');if(!email||!emailLabel)return;
    const usernameLabel=document.createElement('label');usernameLabel.id='staffUsernameLabel';usernameLabel.className='wide';usernameLabel.innerHTML=`${esc(tr('Username'))}<input id="staffUsername" class="field" autocomplete="off">`;emailLabel.before(usernameLabel);const access=document.createElement('label');access.className='wide';access.innerHTML=`<span><input id="staffBackOffice" type="checkbox"> ${esc(tr('Allow Back Office access'))}</span>`;$('staffBranches')?.closest('label')?.after(access);const role=$('staffRole');role.innerHTML=`<option value="Cashier">${esc(tr('Cashier'))}</option><option value="Manager">${esc(tr('Manager'))}</option><option value="Admin">${esc(tr('Admin'))}</option>`;
    const applyMode=()=>{const use=!!state.publicSettings.useUsername;usernameLabel.classList.toggle('identity-hidden',!use);$('staffUsername').required=use;email.required=!use;if(emailLabel.firstChild?.nodeType===Node.TEXT_NODE)emailLabel.firstChild.nodeValue=use?tr('Email (optional)'):tr('Email')};applyMode();
    window.loadStaff=async function(){try{const latest=await window.api('/api/settings');state.publicSettings.useUsername=!!latest.useUsername}catch{}state.branches=await window.api('/api/branches');state.staff=await window.api('/api/staff');const bn=id=>state.branches.find(b=>b.id===id)?.name||id,identity=x=>state.publicSettings.useUsername?(x.username||'—'):(x.email||'—');$('staffTable').innerHTML=`<div class="table-wrap"><table class="table"><thead><tr><th>${esc(state.publicSettings.useUsername?tr('Username'):tr('Email'))}</th>${state.publicSettings.useUsername?`<th>${esc(tr('Email'))}</th>`:''}<th>${esc(tr('Role'))}</th><th>${esc(tr('Back Office access'))}</th><th>${esc(tr('Assigned branches'))}</th><th>${esc(tr('Actions'))}</th></tr></thead><tbody>${state.staff.map(x=>`<tr><td>${esc(identity(x))}</td>${state.publicSettings.useUsername?`<td>${esc(x.email||'—')}</td>`:''}<td>${esc(tr(x.role))}</td><td>${x.canAccessBackOffice?'✓':'—'}</td><td>${esc((x.branchIds||[]).map(bn).join(', ')||tr('Assigned to all branches'))}</td><td><button class="btn ghost small" onclick="editStaff('${x.id}')">${esc(tr('Edit'))}</button> <button class="btn danger small" onclick="deleteStaff('${x.id}')">${esc(tr('Delete'))}</button></td></tr>`).join('')}</tbody></table></div>`;const checks=$('staffBranches');checks.innerHTML=state.branches.map(b=>`<label class="branch-check"><input type="checkbox" value="${b.id}">${esc(b.name)}</label>`).join('');applyMode()};
    ...
    form.onsubmit=async e=>{e.preventDefault();try{const id=$('staffId').value,branchIds=[...$('staffBranches').querySelectorAll('input:checked')].map(x=>x.value),body={username:$('staffUsername').value||null,email:email.value||null,role:role.value,canAccessBackOffice:$('staffBackOffice').checked,password:$('staffPassword').value||(id?null:''),branchIds};await window.api('/api/staff'+(id?'/'+id:''),{method:id?'PUT':'POST',body:JSON.stringify(body)});window.resetStaffForm();await window.loadStaff();window.toast?.(tr('Saved'))}catch(error){window.toast?.(error.message,true)}};
  }

  async function initialize(){
    await loadPublicSettings();if(state.publicSettings.systemLanguage)await window.MercatoUI?.setLanguage?.(state.publicSettings.systemLanguage);await loadLocale();setupLoginArtwork();setupLoginIdentity();setupAdminRouting();setupReferenceHelp();setupProductImageUpload();setupInvoiceCustomerFilter();setupSettings();setupStaff();ensurePrintFooter();syncIdentityDisplay();translate(document.body);
    const observer=new MutationObserver(records=>{records.forEach(r=>{r.addedNodes.forEach(n=>{if(n.nodeType===1)translate(n)});ensurePrintFooter(r.target)});syncIdentityDisplay()});observer.observe(document.body,{childList:true,subtree:true});
  }
