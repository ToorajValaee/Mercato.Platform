...
public sealed class SettlementRepository : ISettlementRepository
{
    ...
    public Task<ArtistSettlement?> GetForPeriodAsync(
        Guid artistId,
        DateTime from,
        DateTime to,
        CancellationToken cancellationToken = default)
    {
        return _context.ArtistSettlements
            .AsNoTracking()
            .FirstOrDefaultAsync(x =>
                x.ArtistId == artistId &&
                x.PeriodFromUtc == from &&
                x.PeriodToUtc == to,
                cancellationToken);
    }

    public async Task<ArtistSettlement> AddSettlementAsync(
    ...
    public async Task<IReadOnlyList<ArtistSettlement>> GetSettlementsAsync(
        Guid? artistId = null,
        bool? isPaid = null,
        CancellationToken cancellationToken = default)
    {
        ...
        return await query
            .OrderByDescending(x => x.PeriodToUtc)
            .ThenBy(x => x.ArtistId)
            .ToListAsync(cancellationToken);
    }

    public async Task<ArtistSettlement?> MarkPaidAsync(
        Guid settlementId,
        DateTime paidAtUtc,
        CancellationToken cancellationToken = default)
    {
        var settlement = await _context.ArtistSettlements
